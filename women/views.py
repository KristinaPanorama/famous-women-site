...
class WomenHome(DataMixin, ListView):
    ...
    def get_queryset(self):
        w_list = cache.get('posts_list')
        if not w_list:
            w_list = Women.published.all().select_related('cat')
            cache.set('posts_list', w_list, 20)
        return w_list
...
def about(request):
    ...
    women_paginator = Paginator(contact_list, 3)

    page_num = request.GET.get('page')
    page = women_paginator.get_page(page_num)
    data = {'title': 'About site', 'page': page}
